...
#define NUM_WAIT_ITEMS NUM_CHANNELS + 2 // add one item for the changed event and one for UART socket
...
//    size_t acl_buffer_packet_length;
...
fail:
printf("hci_handle_uart_read_events fail\n");
    mtx_lock(&hci->mutex);
    channel_cleanup_locked(hci, &hci->acl_channel);
    mtx_unlock(&hci->mutex);
}
...
static int hci_read_thread(void* arg) {
    ...
    while (1) {
        ...
        if (status == ZX_OK) {
            ...
            if (!hci_has_read_channels_locked(hci)) {
                zxlogf(ERROR, "bt-transport-uart: all channels closed - exiting\n");
                break;
            }
        }
    }
